 ...
 * intersect with rounded corners. Instead, it adds padding to avoid such intersection (See {@link
 * #setPreventCornerOverlap(boolean)} to change this behavior).
 * <p>
 * Before L, CardView adds padding to its content and draws shadows to that area. This padding amount is equal to
 ...
 * edges of the Card and children of CardView.
 * <p>
 * Note that, if you specify exact dimensions for the CardView, because of the shadows, its content area will be
 * different between platforms before L and after L. By using api version specific resource values, you can avoid these
 * changes. Alternatively, If you want CardView to add inner padding on platforms L and after as well, you can set
 * {@link #setUseCompatPadding(boolean)} to
 * <code>true</code>.
 * <p>
 * To change CardView's elevation in a backward compatible way, use {@link #setCardElevation(float)}. CardView will use
 * elevation API on L and before L, it will change the shadow size. To avoid moving the View while shadow size is
 * changing, shadow size is clamped by {@link #getMaxCardElevation()}. If you want to change elevation dynamically, you
...
public class CardView extends LinearLayout implements CardViewDelegate {
    ...
    */
	 ...
	 *
	 * @return True CardView adds inner padding on platforms L and after to have same dimensions with platforms before
	 * 		L.
	 */
	@Override
	 ...
	 * <p>
	 * This may cause Cards to have different sizes between L and before L. If you need to align CardView with other
	 * Views, you may need api version specific dimension resources to account for the changes. As an alternative, you
	 * can set this flag to <code>true</code> and CardView will add the same padding values on platforms L and after.
	 * <p>
	 * Since setting this flag to true adds unnecessary gaps in the UI, default value is
	 * <code>false</code>.
	 *
	 * @param useCompatPadding True if CardView should add padding for the shadows on platforms L and above.
	 * @attr ref android.support.v7.cardview.R.styleable#CardView_cardUseCompatPadding
	 */
	 ...
	 * <p>
	 * Depending on platform version or {@link #getUseCompatPadding()} settings, CardView may update these values
	 * before
	 ...
	 * Updates the backward compatible elevation of the CardView.
	 * <p>
	 * Calling this method has no effect if device OS version is L or newer and {@link #getUseCompatPadding()} is
	 * <code>false</code>.
	 *
	 * @param radius The backward compatible elevation in pixels.
	 * @attr ref android.support.v7.cardview.R.styleable#CardView_cardElevation
	...
	/**
	 * Returns whether CardView should add extra padding to content to avoid overlaps with rounded corners on API
	 * versions 20 and below.
	 *
	 * @return True if CardView prevents overlaps with rounded corners on platforms before L. Default value is
	 * 		<code>true</code>.
	 */
	@Override
	...
	/**
	 * On API 20 and before, CardView does not clip the bounds of the Card for the rounded corners. Instead, it adds
	 * padding to content so that it won't overlap with the rounded corners. You can disable this behavior by setting
	 * this field to <code>false</code>.
	 * <p>
