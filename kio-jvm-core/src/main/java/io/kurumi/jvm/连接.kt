...
open class 连接实现 {
    ...
    constructor(_地址: String, _方法: String) : this(_地址) {
        方法 = _方法
    }
    ...
    fun 请求头(_键值: String, _内容: String?, _覆盖: Boolean): 连接实现 {
    ...
    fun 参数(_键值: String, _内容: String?): 连接实现 {
        if (_内容 == null) {
            请求.form().remove(_键值)
        } else {
            请求.form().put(_键值, _内容)
