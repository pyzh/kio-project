...
public class HTTPSession implements IHTTPSession {
	...
	private void decodeHeader(BufferedReader in, Map<String, String> pre, Map<String, List<String>> parms, Map<String,
		...
		try {
			...
			if (!st.hasMoreTokens()) {
				throw new ResponseException(Status.BAD_REQUEST, "BAD REQUEST: Missing URI. Usage: GET /example/file" +
						".html");
			}
	...
	private void decodeMultipartFormData(ContentType contentType, ByteBuffer fbuf, Map<String, List<String>> parms,
		...
		try {
			...
			if (boundaryIdxs.length < 2) {
				throw new ResponseException(Status.BAD_REQUEST, "BAD REQUEST: Content type is multipart/form-data but " +
						"contains less than two boundary strings.");
			}
			...
			for (int boundaryIdx = 0; boundaryIdx < boundaryIdxs.length - 1; boundaryIdx++) {
				...
				if (mpline == null || !mpline.contains(contentType.getBoundary())) {
					throw new ResponseException(Status.BAD_REQUEST, "BAD REQUEST: Content type is multipart/form-data " +
							"but chunk does not start with boundary.");
				}
	...
	public void execute() throws IOException {
		...
		try {
			...
			boolean keepAlive = "HTTP/1.1".equals(protocolVersion) && (connection == null || !connection.matches("(?i)" +
					".*close.*"));
			...
			if (r == null) {
				throw new ResponseException(Status.INTERNAL_ERROR, "SERVER INTERNAL ERROR: Serve() returned a null " +
						"response.");
			} else {
			...
			Response resp = Response.newFixedLengthResponse(Status.INTERNAL_ERROR, NanoHTTPD.MIME_PLAINTEXT, "SSL " +
					"PROTOCOL FAILURE: " + ssle.getMessage());
			resp.send(this.outputStream);
			NanoHTTPD.safeClose(this.outputStream);
		} catch (IOException ioe) {
			Response resp = Response.newFixedLengthResponse(Status.INTERNAL_ERROR, NanoHTTPD.MIME_PLAINTEXT, "SERVER " +
	...
	/**
	 * Find the byte positions where multipart boundaries start. This reads a large block at a time and uses a
	 * temporary
	 * buffer to optimize (memory mapped) file access.
	 */
		...
		try {
			...
			if (Method.POST.equals(this.method)) {
				...
				if (contentType.isMultipart()) {
					...
					if (boundary == null) {
						throw new ResponseException(Status.BAD_REQUEST, "BAD REQUEST: Content type is " +
